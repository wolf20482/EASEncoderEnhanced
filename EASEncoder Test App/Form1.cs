...
namespace EASEncoder_Test_App
{
    public partial class Form1 : Form
    {
        ...
        {
                Form2 customEncode = new Form2();
                if (customEncode.ShowDialog(this) == DialogResult.OK)
                {
                    EASEncoder.EASEncoder.CreateNewMessageFromRawData(message: customEncode.txtCustomData.Text, ebsTone: customEncode.checkBoxEBS.Checked, nwsTone: customEncode.checkBoxNWR.Checked, announcement: formatAnnouncement(customEncode.txtAnnouncement.Text), filename: customEncode.txtOutputFile.Text);
                }
                else
                {
                }
